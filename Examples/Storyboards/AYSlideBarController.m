...
- (void)viewDidLoad {
    ...
    effectView.frame = _containerView.bounds;
    effectView.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin
                                    | UIViewAutoresizingFlexibleWidth
                                    | UIViewAutoresizingFlexibleTopMargin
                                    | UIViewAutoresizingFlexibleHeight;
    [_containerView addSubview:effectView];
...
- (void)setContentViewController:(UIViewController *)contentViewController {
    ...
    _contentViewController = contentViewController;
    [self ay_addController:_contentViewController frame:self.view.bounds];
    [_contentViewController.view.superview insertSubview:_contentViewController.view belowSubview:_containerView];
...
- (void)updateContainerFrame {
    if (self.automaticSize) {
        if (self.position == AYSlideBarControllerPositionLeft || self.position == AYSlideBarControllerPositionRight)
            self.calculatedMenuViewSize = CGSizeMake(self.contentViewController.view.frame.size.width - 50.0f, self.view.bounds.size.height);
        
        if (self.position == AYSlideBarControllerPositionTop || self.position == AYSlideBarControllerPositionBottom)
            self.calculatedMenuViewSize = CGSizeMake(self.contentViewController.view.frame.size.width, self.view.bounds.size.height - 50.0f);
    } else {
